...
describe('GET /api/v1/urls', () => {
  it('pages newest-first through cursor chains with no total count', async () => {
    ...
    const created = await createMany(api, 5);
    ...
    const page1 = await api.get('/api/v1/urls?limit=2');
  ...
  it('searches across code, destination, and alias', async () => {
    ...
    const empty = await api.get('/api/v1/urls?search=no-such-thing');
  ...
  it('rejects bad pagination input with 400', async () => {
    ...
    for (const query of ['?limit=0', '?limit=101', '?cursor=garbage!!']) {
      const res = await api.get(`/api/v1/urls${query}`);
  ...
  it('matches LIKE wildcards literally', async () => {
    const { api } = await authedClient();
    await api.post('/api/v1/urls').send({ url: 'https://example.com/100%_coverage' });

    const res = await api.get('/api/v1/urls?search=100%25_coverage');
    expect(res.status).toBe(200);
...
describe('GET /api/v1/urls/:shortCode', () => {
  it('returns details with lifetime clicks and shortUrl', async () => {
    const { api } = await authedClient();
    const created = await api.post('/api/v1/urls').send({ url: 'https://example.com/d' });
    ...
    const res = await api.get(`/api/v1/urls/${code}`);
    expect(res.status).toBe(200);
  ...
  it('answers 404 for unknown codes', async () => {
    const { api } = await authedClient();
    const res = await api.get('/api/v1/urls/never');
